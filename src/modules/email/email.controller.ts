...
export class EmailController {
  ...
  @Get(':filePath')
  async getEmailData(@Param('filePath') filePath: string, @Res() res: Response): Promise<void> {
    try {
  ...
  private findJsonAttachment(emailData: any): any | null {
    return emailData.attachments?.find(attachment => attachment.contentType === 'application/json') || null;
  }

  private respondWithJsonAttachment(res: Response, jsonAttachment: any): void {
    res.set('Content-Type', 'application/json');
    res.set('Content-Disposition', `attachment; filename=${jsonAttachment.filename}`);
    res.send(jsonAttachment.content.toString('utf-8'));
  }

  private async respondWithJsonUrl(res: Response, content: string | false | null): Promise<void> {
    if (typeof content === 'string' && content) {
      ...
      if (jsonUrl) {
        ...
        throw new HttpException('Failed to extract JSON URL from the email.', HttpStatus.INTERNAL_SERVER_ERROR);
      }
    } else {
  ...
  private handleErrorResponse(res: Response, error: any): void {
    if (error instanceof HttpException) {
      ...
      res.status(HttpStatus.INTERNAL_SERVER_ERROR).json({ error: 'Unexpected error occurred.' });
    }
  }
}
